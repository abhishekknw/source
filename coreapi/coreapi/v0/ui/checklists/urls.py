...
from views import (CreateChecklistTemplate, ChecklistEntry, GetCampaignChecklists,
                   ...
                   DeleteChecklistRow, ChecklistEdit, FreezeChecklist, ChecklistPermissionsAPI)
...
urlpatterns = [
    ...
    url(r'^(?P<checklist_id>[A-Z_a-z0-9]+)/freeze/(?P<state>[0-1]+)$', FreezeChecklist.as_view()),
    url(r'^permissions/(?P<organisation_id>[A-Z_a-z0-9]+)/$', ChecklistPermissionsAPI.as_view()),
]